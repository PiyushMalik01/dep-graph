...
- 2026-09-16 (eval pass, no LLM credits used per user): hand-labelled 125 dev edges (baseline ~55% weighted precision: documented 88%, structural 57%, heuristic 30%), fixed the error classes (source requires the slot it provides, incidental-word heuristics, scope mismatch, LIST_PUBLIC_* sources, "instead"/"to check" doc refs, ADD_*_TO_* mutations, two-word entities), then labelled a 110-edge held-out sample drawn after tuning: documented 100%, structural 80%, heuristic 73%, ~80% overall. `bun run eval` scores both sets and asserts readme examples. Viewer: numbered "How to run it" plans per tool, doc quotes on documented edges, accuracy table, service-map internal link counts.
